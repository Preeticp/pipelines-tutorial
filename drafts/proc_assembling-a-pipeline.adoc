...
`Pipeline` s represent a CI/CD flow and are defined by the `Tasks` to be executed.  They specify how the `Tasks` interact with each other and their order of execution using the `inputs` , `outputs`, and the `run-After` parameters. They are designed to be generic and reusable in multiple applications and environments.
